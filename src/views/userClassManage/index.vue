<template>
  <div class="app-container">
    ...
    <el-table
      ...
      <el-table-column class-name="status-col" label="权限名称"  align="center">
        <template slot-scope="scope">
...
export default {
  ...
  methods: {
    handleUpdate(id) {
      this.$router.push({ path: `/userClassManage/update/${id}` })
    },
    handleDelete(id) {
    ...
    handleCreate() {
      this.$router.push({ name: 'CreateUserClass' })
    },
    ...
    getList() {
      this.loading = true
      author.query(this.listQuery)
        .then((result) => {
          this.author = result.data.list
